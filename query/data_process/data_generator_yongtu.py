...
def gen_by_rule(num):
    ...
    for i in range(num):
        ...
        if mode <= 0.35:
            ...
            part_a = random.choice(['有','作为','可作']) + random.choice(['什么', '啥', '什么']) + random.choice(
                ['用途','用处','功用','用场'])
            ...
            if mode2 == 1:
                ...
                part_b = random.choice(PLEASE_DO) + random.choice(PREFIX_DO) + random.choice(DO)
            part_a = random.choice(['可以', '主要','广泛','常']) +   random.choice(
                ['用作','用来','用以'])+random.choice(['干', '做', '']) +random.choice(['什么', '啥', '什么'])
            part_c = random.choice(SUFFIX_)
        else:
            mode2 = random.randint(0, 5)
            if mode2 == 1:
                ...
                part_b = random.choice(PLEASE_DO) + random.choice(PREFIX_DO) + random.choice(DO)
            part_a = "的"+ random.choice(['用途', '用处', '功用', '用场'])+random.choice(['是']) + \
                      random.choice(['什么', '啥', '什么'])
...
if __name__ == '__main__':
    ...
    write(positive, negative, '../../data/yongtu.txt', '../../data/chaodai-other.txt')
