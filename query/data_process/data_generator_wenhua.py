...
def gen_by_rule(num):
    ...
    for i in range(num):
        ...
        if mode <= 0.85:
            ...
            part_a = random.choice(['的', '']) +random.choice(['文化', ''])+random.choice(['是', '为','属于','产生于']) + random.choice(['什么', '啥', '什么','哪个'])
            ...
            part_a =  random.choice(
                [ '属于','属于','归为']) + random.choice(['什么', '哪个', '什么', '哪个'])+ random.choice(['文化', ''])
            part_c = random.choice(SUFFIX_)
...
if __name__ == '__main__':
    ...
    write(positive, negative, '../../data/wenhua.txt', '../../data/chaodai-other.txt')
