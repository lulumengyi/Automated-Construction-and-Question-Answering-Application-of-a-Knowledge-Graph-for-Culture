...
def gen_by_rule(num):
    ...
    for i in range(num):
        ...
        if mode <= 0.45:
            ...
            part_a = random.choice(['是', '为', '划分为','属于','归为','算作']) + random.choice(['哪个', '哪种','什么']) + random.choice(
                ['类型','类别','类'])
            ...
            if mode2 == 1:
                ...
                part_b = random.choice(PLEASE_DO) + random.choice(PREFIX_DO) + random.choice(DO)
            part_a = random.choice(['是']) +  random.choice(
                ['一种',''])+ random.choice(
                ['什么']) + random.choice(
                ['类型','类别','类'])+random.choice(['的文物'])
            part_c = random.choice(SUFFIX_)
        else:
            mode2 = random.randint(0, 5)
            if mode2 == 1:
                ...
                part_b = random.choice(PLEASE_DO) + random.choice(PREFIX_DO) + random.choice(DO)
            part_a = random.choice(['为', '是']) +  random.choice(['何种', '哪种']) +"文物"
...
if __name__ == '__main__':
    ...
    write(positive, negative, '../../data/leixing.txt', '../../data/chaodai-other.txt')
