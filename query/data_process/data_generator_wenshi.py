...
def gen_by_rule(num):
    ...
    for i in range(num):
        ...
        if mode <= 0.35:
            ...
            part_a = random.choice(['包含', '有',  '有']) + random.choice(['什么', '啥', '什么']) + random.choice(
                ['纹饰', '装饰', '图案', '饰样', '花纹','饰纹', '图饰', '图纹', '纹理', '纹案'])
            ...
            part_a = random.choice(['的', '']) +  random.choice(
                ['纹饰', '装饰', '图案', '饰样', '花纹','饰纹', '图饰', '图纹', '纹理', '纹案'])+ random.choice(
                ['包含', '有', '是', '为', '着', '是']) + random.choice(['什么', '啥', '什么'])
            part_c = random.choice(SUFFIX_)
        else:
            mode2 = random.randint(0, 5)
            if mode2 == 1:
                ...
                part_b = random.choice(PLEASE_DO) + random.choice(PREFIX_DO) + random.choice(DO)
            part_a = random.choice(['装饰', '饰以', '有', '纹']) + random.choice(
                ['着', '']) + random.choice(['什么', '啥', '什么']) +random.choice(
                ['纹饰', '装饰', '图案', '饰样', '花纹','饰纹', '图饰', '图纹', '纹理', '纹案'])
            part_c = random.choice(SUFFIX_)
...
if __name__ == '__main__':
    ...
    write(positive, negative, '../../data/wenshi.txt', '../../data/chaodai-other.txt')
