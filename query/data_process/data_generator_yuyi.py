...
def gen_by_rule(num):
    ...
    for i in range(num):
        ...
        if mode <= 0.35:
            ...
            part_a =  random.choice(['包含', '有', '是', '为', '有'])+ random.choice(['什么', '啥', '什么']) +random.choice(['寓意', '涵义', '意义', '喻意', '含义'])
            ...
            if mode2 == 1:
                ...
                part_b = random.choice(PLEASE_DO) + random.choice(PREFIX_DO) + random.choice(DO)
            part_a =  random.choice(['的', ''])  + random.choice(['寓意', '涵义', '意义', '喻意', '含义']) + random.choice(['包含', '有', '是', '为', '着', '是'])+ random.choice(['什么', '啥', '什么'])
            part_c = random.choice(SUFFIX_)
        else:
            mode2 = random.randint(0, 5)
            if mode2 == 1:
                ...
                part_b = random.choice(PLEASE_DO) + random.choice(PREFIX_DO) + random.choice(DO)
            part_a =  random.choice(['象征', '喻指', '意味', '喻示', '寓为', '寓意','寓为'])+ random.choice(['着',''])+ random.choice(['什么', '啥', '什么'])
            part_c = random.choice(SUFFIX_)
...
if __name__ == '__main__':
    ...
    write(positive, negative, '../../data/yuyi.txt', '../../data/chaodai-other.txt')
